...
public class Base64 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Encoded byte array: " + string);

        // convert them back to byte array
        byte[] newBytes = Base64.decode(string);

        // check if new byte array is correct
        assert bytes.length == newBytes.length;
        for (int i = 0; i < bytes.length; i++) {
            assert (bytes[i] == newBytes[i]);
            System.out.println("bytes[" + i + "]: " + bytes[i] + " == newBytes[" + i + "]: " + newBytes[i]);
        }
    }
     ...
     * decodeToObject(String encodedObject, ClassLoader loader) with loader =
     * null;
     */
